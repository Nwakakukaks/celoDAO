...
contract("celoDAO Contribution Test", accounts => {

  it("should receive contribution", async () => {
    const instance = await celoDAO.new();
    ...
    const celoBalance = await web3.eth.getBalance(contractAddress);

    assert.equal(celoBalance, amount);
  });
